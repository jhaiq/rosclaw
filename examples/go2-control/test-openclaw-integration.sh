...
GO2_NODE=$(docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros node list" 2>&1 | grep unitree_go2_node || true)
if [[ -z "$GO2_NODE" ]]; then
    echo "[INFO] Starting GO2 node in container..."
    docker exec -d "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && python3 /agiros_ws/src/unitree_go2/unitree_go2/go2_node.py"
    sleep 3
    GO2_NODE=$(docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros node list" 2>&1 | grep unitree_go2_node || true)
    if [[ -z "$GO2_NODE" ]]; then
...
TOPICS=$(docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros topic list" 2>&1 | grep -E "(go2_|cmd_vel)" || true)
if [[ -z "$TOPICS" ]]; then
...
docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros topic pub /go2_command/stand std_msgs/msg/Empty --once"
echo "[OK] Stand command published"
echo ""

# Test publishing velocity command
echo "[4/5] Testing velocity command..."
docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros topic pub /cmd_vel geometry_msgs/msg/Twist '{linear: {x: 0.5}, angular: {z: 0.0}}' --once"
sleep 1
...
if [[ -n "$OPENCLAW_CONTAINER" ]]; then
    ...
    # Check AGIROS transport status from logs
...
ROSAPI_TOPICS=$(docker exec "$ROS2_CONTAINER" bash -c "source /opt/agiros/pixiu/setup.sh && source /agiros_ws/install/setup.sh && agiros service call /rosapi/topics rosapi_msgs/srv/Topics '{}'" 2>&1 | grep "topics:" || echo "")
