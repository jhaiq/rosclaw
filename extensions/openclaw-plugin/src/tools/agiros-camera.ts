...
/**
 * Register the agiros_camera_snapshot tool with the AI agent.
 * Grabs a single frame from a camera topic.
 */
export function registerCameraTool(api: OpenClawPluginApi): void {
  api.registerTool({
    name: "agiros_camera_snapshot",
    label: "AGIROS  Camera Snapshot",
    description:
      "Capture a single image from a AGIROS camera topic. Returns the image as base64-encoded data. " +
