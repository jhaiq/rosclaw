...
/**
 * Register the agiros_subscribe_once tool with the AI agent.
...
export function registerSubscribeTool(api: OpenClawPluginApi): void {
  api.registerTool({
    name: "agiros_subscribe_once",
    label: "AGIROS  Subscribe Once",
    description:
      "Subscribe to a AGIROS topic and return the next message. Use this to read sensor data, " +
      "check robot state, or get the current value of a topic.",
    parameters: Type.Object({
      topic: Type.String({ description: "The AGIROS topic name (e.g., '/battery_state')" }),
      type: Type.Optional(Type.String({ description: "The AGIROS message type (e.g., 'sensor_msgs/msg/BatteryState')" })),
