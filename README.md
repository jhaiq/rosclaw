...
**Natural language control of AGIROS robots through messaging apps, powered by AI agents.**
...
Whethere it's a cute desk robot or a humanoid robot, all you have to do is install our OpenClaw extension and run our AGIROS packakge.
...
2. OpenClaw's AI agent receives the message and uses AGIROS tools registered by the RosClaw plugin
3. The agent translates intent into AGIROS operations (topic publish, service call, action goal)
...
├── agiros_ws/src/
│   ├── rosclaw_discovery/        # AGIROS capability auto-discovery node
│   └── rosclaw_msgs/             # Custom AGIROS message/service definitions
...
This starts AGIROS + rosbridge + Gazebo. Then configure your OpenClaw instance to use the RosClaw plugin with `ws://localhost:9090`.
...
| [`@rosclaw/openclaw-plugin`](extensions/openclaw-plugin/) | OpenClaw extension: tools, hooks, skills, commands for AGIROS control |
| [`@rosclaw/openclaw-canvas`](extensions/openclaw-canvas/) | Real-time robot dashboard (Phase 3) |
| [`rosclaw_discovery`](agiros_ws/src/rosclaw_discovery/) | AGIROS Python node for capability auto-discovery |
| [`rosclaw_msgs`](agiros_ws/src/rosclaw_msgs/) | Custom AGIROS message/service definitions |

## Agent Tools

The AI agent has access to these AGIROS tools:

| Tool | Description |
|---|---|
| `agiros_publish` | Publish messages to any AGIROS topic |
| `agiros_subscribe_once` | Read the latest message from a topic |
| `agiros_service_call` | Call a AGIROS service |
| `agiros_action_goal` | Send action goals with feedback (Phase 2) |
| `agiros_param_get/set` | Get/set AGIROS node parameters |
| `agiros_list_topics` | Discover available topics |
| `agiros_camera_snapshot` | Capture a camera frame |
